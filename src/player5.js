...
export default function Player5() {
  ...
  const rollDice = () => {
    ...
    // Set playerTurn text and update positions for five players
    if (tog % 5 === 0) {
      ...
      setPlayerTurn("yellow's");
      setTurnColor('yellow');
      if (p4 + result <= 100) {
        // Update positions for Player 4
        if (p4 > 0) {
          let grid_id = `g${p4}`;
          let element = document.getElementById(grid_id);
          element.innerHTML = "";
        ...
        let finalresult = snakeladder(p4 + result, 'yellow');
        setP4(finalresult);
        let grid_id = `g${finalresult}`;
        let element = document.getElementById(grid_id);
        element.innerHTML = `<img src=${yellow} height="30vw" width="30vw" />`;

        if (finalresult === 100) {
          setTimeout(() => alert("yellow won the game"), 1000);
        }
      }
    } else {
      setPlayerTurn("skyblue's");
      setTurnColor('skyblue');
      if (p5 + result <= 100) {
        // Update positions for Player 5
        if (p5 > 0) {
          let grid_id = `g${p5}`;
          let element = document.getElementById(grid_id);
          element.innerHTML = "";
        ...
        let finalresult = snakeladder(p5 + result, 'skyblue');
        setP5(finalresult);
        let grid_id = `g${finalresult}`;
        let element = document.getElementById(grid_id);
        element.innerHTML = `<img src=${skyblue} height="30vw" width="30vw" />`;

        if (finalresult === 100) {
          setTimeout(() => alert("skyblue won the game"), 1000);
        }
      }
    }
